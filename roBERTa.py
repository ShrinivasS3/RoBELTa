...
from transformers import AutoModel, AutoTokenizer, BertModel, BatchEncoding, PreTrainedTokenizerBase, RobertaModel


class RobertaClassifier(ABC):
    ...
    def __init__(
        ...
        if not neural_network:
            roberta = AutoModel.from_pretrained(pretrained_model_name_or_path)
            neural_network = RobertaClassifierNN(roberta)
    ...
    @classmethod
    def load(cls, model_dir: str, device: str = "cuda:0", many_gpus: bool = False) -> RobertaClassifier:
        model_dir = Path(model_dir)
...
class RobertaClassifierNN(Module):
